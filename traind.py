...
if __name__ == '__main__':
    if not (ServiceBusTaskSource.is_supported() or StorageQueueTaskSource.is_supported()):
        print('No supported task sources configured. Please set environment '
              'variables required for either ServiceBusTaskSource or '
              'StorageQueueTaskSource.')
        exit(10)
    ...
    while True:
        ...
        for task in tasks:
            with TemporaryDirectory() as sandbox:
                ...
                if exit_code:
                    print('Non-zero exit code from training, so not '
                          'comitting task.')
                else:
